use crate::{builder::Builder, models::content::HasRouteSegment, XivApi};
use serde::Serialize;
use std::borrow::Cow;
...
impl<'x, 'a, O> Builder<'x> for ContentBuilder<'x, 'a, O>
where
    O: HasRouteSegment,
...
impl<'x, 'a, O> ContentBuilder<'x, 'a, O>
where
    for<'de> O: serde::Deserialize<'de>,
{
    pub(crate) fn new(api: &'x XivApi, id: u64) -> Self {
        ContentBuilder {
            api,
            id,
            minified: None,
